...
class EnhancedResonanceFlux(nn.Module):
    ...
    def forward(self, k_chunk, u_chunk):
        ...
        if len(k_chunk.shape) == 4:
            # Original shape: [b, h, c, d_k]
            b, h, c, _ = k_chunk.shape
            
            # Bilinear attention between keys and values
            attn_scores = torch.matmul(
                k_chunk, 
                torch.matmul(self.W_bilinear, u_chunk.transpose(-1, -2))
            )
            
            attn_scores = attn_scores / self.temp.view(1, h, 1, 1)
            avg_attn = torch.mean(attn_scores, dim=(-2, -1))  # [b, h]
            k_avg = k_chunk.mean(dim=2)  # [b, h, d_k]
            u_avg = u_chunk.mean(dim=2)  # [b, h, d_v]
        else:
            # Vectorized shape: [b*h*n, c, d_k]
            batch_combined, c, _ = k_chunk.shape
            ...
            # FIX #3: Properly replicate temperature for all heads
            if batch_combined % self.num_heads == 0:
                num_batches_chunks = batch_combined // self.num_heads
                # Replicate: [h] -> [b*n, h] -> [b*h*n]
                repeated_temp = self.temp.unsqueeze(0).repeat(num_batches_chunks, 1).view(-1)
                attn_scores = attn_scores / repeated_temp.view(-1, 1, 1)
            else:
                # Fallback if structure doesn't match
                attn_scores = attn_scores / self.temp[0]
            
            avg_attn = torch.mean(attn_scores, dim=(-2, -1))  # [b*h*n]
            k_avg = k_chunk.mean(dim=1)  # [b*h*n, d_k]
            u_avg = u_chunk.mean(dim=1)  # [b*h*n, d_v]
...
def _enhanced_hierarchical_delta_rule_impl(
    ...
    psi_all = resonance_flux(k_reshaped, u_reshaped)  # [b*h*n] - scalar per batch*head*chunk
    ...
    for i in range(num_chunks):
        ...
        psi_i = psi_all[:, :, i]  # Pre-computed resonance flux
        ...
        flux_weight = psi_i.unsqueeze(-1).unsqueeze(-1)  # [b, h, 1, 1]
        alpha = 0.5 + 0.3 * flux_weight
