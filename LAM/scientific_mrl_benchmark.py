...
try:
    ...
    print("❌ Could not import LAM from test_8k_LAM.py")
    sys.exit(1)
...
class LongContextValidator:
    ...
    def test_length_scaling(self, token_lengths=[512, 1024, 2048, 4096, 8192, 16384, 32768], use_half_precision=True):
        """
        TEST 3: LONG CONTEXT VALIDATION (32K Tokens)
        =============================================
        
        What Are We Testing?
        --------------------
        1. Does LAM's position interpolation actually work for long sequences?
        2. Can it encode 32K tokens without crashing or producing garbage?
        ...
        2. Generate longer documents at increasing lengths (512 to 32K tokens)
        ...
        - Long document (32K tokens, same topic)
        ...
        -----------------------
        We're NOT testing if 32K embeddings are BETTER than 512-token embeddings.
        We're testing if they're MEANINGFUL (not corrupted).
        
        What Good Looks Like:
        ---------------------
        All lengths from 512 to 32,768 tokens:
        ...
        """
        print("\n🧪 TEST 3: LONG CONTEXT VALIDATION (Up to 32K tokens)")
        print("="*80)
        ...
        if hasattr(self.model, 'model'):
            # LAM wrapper - use internal model's get_sentence_embeddings for raw 384-dim
            with torch.no_grad():
        ...
        if use_half_precision and torch.cuda.is_available():
            ...
            if supports_bf16:
                ...
                model_half = self.model.model.bfloat16() if hasattr(self.model, 'model') else self.model.bfloat16()
                autocast_dtype = torch.bfloat16
            else:
                # Create a copy in float16 for long sequences
                model_half = self.model.model.half() if hasattr(self.model, 'model') else self.model.half()
        ...
        for length in token_lengths:
            try:
                ...
                # This aligns with training script and global evaluation
                if use_half_precision and model_half is not None and length >= 8192:
                    # Temporarily use half precision model
                    if hasattr(self.model, 'model'):
                    ...
                    start = time.time()
                    with torch.no_grad():
                        with torch.amp.autocast('cuda', dtype=autocast_dtype):  # Updated to new API
                            emb = encode_func(tokens['input_ids'], tokens['attention_mask'])
                    elapsed = (time.time() - start) * 1000
                    
                    # Convert to float32 for comparison with base_emb
                    emb = emb.float()
                    
                    # Restore original model (it should already be in float32)
                    if hasattr(self.model, 'model'):
                        self.model.model = original_model
                        # Ensure it's in float32
                        if next(self.model.model.parameters()).dtype != torch.float32:
                            self.model.model = self.model.model.float()
                    else:
                    ...
                    # Use full precision for shorter sequences
                    # Ensure model is in FP32 state (not half precision)
                    if hasattr(self.model, 'model'):
                        # Make sure model is in float32
                        if next(self.model.model.parameters()).dtype != torch.float32:
                            self.model.model = self.model.model.float()
                        encode_func = self.model.model.get_sentence_embeddings
                    else:
...
def main():
    ...
    RUN_RETRIEVAL_TEST = True          # TEST 2: Retrieval performance at scale
    RUN_LONG_CONTEXT_TEST = True
    ...
    if RUN_LONG_CONTEXT_TEST:
        ...
        long_context_results = validator.test_length_scaling(
            token_lengths=[512, 1024, 2048, 4096, 8192, 16384, 32768],
