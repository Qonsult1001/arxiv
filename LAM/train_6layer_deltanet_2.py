...
config = {
    ...
    "max_length": 128,  # Sequence length (MAX: 512 - limited by base model's positional embeddings)
    ...
    "resume_from_step": 156000,  # Resume from checkpoint_50000.pt (highest score) -(76000)
    "log_interval": 50,
    "save_interval": 1000,
    "eval_interval": 2000,  # ⭐ Evaluate validation every N steps (save best as pytorch_model.bin)
    
    # Output directory
    "output_dir": "/workspace/LAM/deltanet_matryoshka",
}
...
class DeltaNetPure6Layer(nn.Module):
    ...
    def get_extended_embeddings(self, input_ids):
        """
        Get embeddings for sequences longer than 512 tokens by interpolating position embeddings.
        Supports sequences up to 8096 tokens in latent space.
        """
        batch_size, seq_len = input_ids.shape
        
        # Get word embeddings (no position limit)
        word_embeddings = self.embeddings.word_embeddings(input_ids)
        
        # Get token type embeddings (usually all zeros for sentence transformers)
        ...
        if seq_len <= 512:
            ...
            # Long sequence: interpolate position embeddings
            # Original positions: 0 to 511
            # Target positions: 0 to seq_len-1
            # We'll use linear interpolation
            original_max_pos = 512
            scale_factor = (original_max_pos - 1) / (seq_len - 1)
            
            # Create interpolated position embeddings
    ...
    def forward_student(self, input_ids, attention_mask):
        ...
        # Use extended embeddings for long sequence support
    ...
    def forward_teacher(self, input_ids, attention_mask):
        ...
        with torch.no_grad():
            ...
            # For long sequences (>512), we need to manually process
            if seq_len > 512:
                # Use extended embeddings
                x = self.get_extended_embeddings(input_ids)
                
                # Manually process through teacher layers
                teacher_hidden_states = []
                for layer in self.teacher_model.encoder.layer:
                    # Self-attention
                    ...
                    teacher_hidden_states.append(x)
                
                # Final pooling
                embeddings_raw = self.mean_pooling(x, attention_mask)
                embeddings_raw = F.normalize(embeddings_raw, p=2, dim=1)
            else:
                # Standard case: use teacher model directly
                ...
                embeddings_raw = F.normalize(embeddings_raw, p=2, dim=1)
                teacher_hidden_states = list(outputs.hidden_states[1:])  # Skip embedding layer
    ...
    def encode(self, input_ids, attention_mask):
        """
        Inference-only method: Pure latent space processing, no teacher.
        This is what should be used for benchmarking and production inference.
        Supports sequences up to 8096 tokens via position embedding interpolation.
        """
        with torch.no_grad():
            # Get embeddings with long sequence support
            x = self.get_extended_embeddings(input_ids)
            
            # Process through all 6 DeltaNet layers (pure latent space)
            for i in range(6):
                # Attention block (all in latent space)
                residual = x
                x_attn, _, _, _ = self.deltanet_layers[i](x, attention_mask)
                x = self.deltanet_norms[i](residual + x_attn)
                
                # FFN block (all in latent space)
                residual = x
                x_ffn = self.deltanet_ffns[i](x)
                x_ffn = F.gelu(x_ffn)
                orig_layer = self.teacher_model.encoder.layer[i]
...
def train():
    ...
    print(f"\n⭐ SEMANTIC KERNEL BLENDING:")
    if config.get('use_kernel_blending', False):
        kernel_path = config.get('kernel_path', 'auto-detect')
        print(f"  ✅ ENABLED - Blending kernel with teacher embeddings during training")
        print(f"  ⭐ Kernel blend alpha: {config.get('kernel_blend_alpha', 0.40)}")
        print(f"  ⭐ Kernel path: {kernel_path}")
        print(f"  💡 Target: Improve raw student score (not blended student)")
    else:
        print(f"  ⚪ DISABLED - Using raw teacher embeddings")
        print(f"  💡 Set 'use_kernel_blending: True' to enable")
